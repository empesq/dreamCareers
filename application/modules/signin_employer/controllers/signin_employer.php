...
class Signin_employer extends MX_Controller 
{
...
public function login() {    
      ...
      if ($this->form_validation->run() == FALSE) {        
          $this->load->view('signin_employer/view_signin');
        }
      else {        
           ...
           if ($query->num_rows() == 1) {          
                ...
                foreach ($query->result() as $row) {            
                    ...
                    if ($hash != $row->hash) {              
                        ...
                        $data = array(
                            'user_id' => $row->employerId,
                            'user_email' => $row->email,
                            'user_name' => $row->employerName,
                            'logged_in' => TRUE,
                            'user_type' => 'employer'
                        );
    ...
    function loggedin() {
        if ($this->session->userdata('logged_in') == TRUE) {
            $this->load->view('signin_employer/view_loggedin');
            //redirect('home/index');
            
        } 
        else {
            redirect('signin_employer');
        } 
    }
